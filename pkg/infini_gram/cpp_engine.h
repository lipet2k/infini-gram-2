...
template <typename T = U16> struct DistResult {
  U64 prompt_cnt;
...
template <typename T = U16> struct InfgramDistResult {
  U64 prompt_cnt;
  map<T, DistTokenResult> result_by_token_id;
  bool approx;
  U64 suffix_len;
};
template <typename T = U16> struct DocResult {
  U64 doc_ix;
  U64 doc_len;
  U64 disp_len;
...
template <typename T = U16> struct SearchDocsResult {
  U64 cnt;
  bool approx;
  vector<U64> idxs;
...
template <typename T = U16> class EngineDiff;

template <typename T = U16> class Engine {

public:
  Engine(const vector<string> index_dirs, const T eos_token_id,
         const T vocab_size, const size_t version, const bool load_to_ram,
         const size_t ds_prefetch_depth, const size_t sa_prefetch_depth,
         const size_t od_prefetch_depth, const set<T> bow_ids,
         const size_t attribution_block_size,
         const bool precompute_unigram_logprobs,
      ...
      : _eos_token_id(eos_token_id), _vocab_size(vocab_size), _version(version),
        _load_to_ram(load_to_ram), _ds_prefetch_depth(ds_prefetch_depth),
        _sa_prefetch_depth(sa_prefetch_depth),
        _od_prefetch_depth(od_prefetch_depth), _bow_ids(bow_ids),
        _attribution_block_size(attribution_block_size), _doc_sep_id((T)(-1)),
        _doc_sep(vector<U8>(sizeof(T), 0xff)) {
    ...
    for (const auto &index_dir : index_dirs) {
      if (prev_shards_by_index_dir.find(index_dir) !=
          prev_shards_by_index_dir.end()) {
        size_t num_prev = prev_shards_by_index_dir[index_dir].size();
        _shards.insert(_shards.end(),
                       prev_shards_by_index_dir[index_dir].begin(),
                       prev_shards_by_index_dir[index_dir].end());
        // Load bigram/trigram/quadgram/unigram caches for prev shards (they are
        // small and cheap to reload)
      ...
      for (size_t s = 0; s < ds_paths.size(); s++) {
        ...
        if (mt_paths.size() == 0) {
          auto shard =
              DatastoreShard{ds, sa, tok_cnt, ds_size, ptr_size, od, doc_cnt};
  ...
  pair<U8 *, U64> load_file(const string &path) {
    if (_load_to_ram) {
      ...
      assert(fstat_ret != -1);
      U8 *ptr = static_cast<U8 *>(
          mmap(NULL, s.st_size, PROT_READ, MAP_PRIVATE, f, 0));
  ...
  map<T, U64> compute_unigram_counts(const size_t s) const {
    ...
    for (size_t i = 0; i < (size_t)_vocab_size + 1; i++) {
      ...
      unigram_counts[input_ids[i]] =
          segment_by_token[i].second - segment_by_token[i].first;
    }
  ...
  FindResult find(const vector<T> input_ids) const {
    ...
    for (size_t s = 0; s < _num_shards; s++) {
      ...
      if (!found_hint && input_ids.size() >= 4 && _has_quadgram_cache[s]) {
        ...
        hint_segment_by_shard.push_back(it != _quadgram_caches[s].end()
                                            ? it->second
                                            : make_pair((U64)0, (U64)0));
        found_hint = true;
      }
      ...
      if (!found_hint && input_ids.size() >= 3 && _has_trigram_cache[s]) {
        ...
        hint_segment_by_shard.push_back(it != _trigram_caches[s].end()
                                            ? it->second
                                            : make_pair((U64)0, (U64)0));
        found_hint = true;
      }
      ...
      if (!found_hint && input_ids.size() >= 2 && _has_bigram_cache[s]) {
        ...
        hint_segment_by_shard.push_back(it != _bigram_caches[s].end()
                                            ? it->second
                                            : make_pair((U64)0, (U64)0));
        found_hint = true;
      }
  ...
  FindResult _find(const vector<T> &input_ids,
                   const vector<pair<U64, U64>> &hint_segment_by_shard) const {
    ...
    vector<thread> threads;
    for (size_t s = 0; s < _num_shards; s++) {
      threads.emplace_back(&Engine::_find_thread, this, s, input_buf, num_bytes,
                           hint_segment_by_shard[s], &segment_by_shard[s]);
    }
    for (auto &thread : threads) {
      thread.join();
    }
  ...
  void _find_thread(const size_t s, const U8 *const input_buf,
                    const U64 num_bytes, const pair<U64, U64> hint_segment,
    ...
    while (r - l > 1) {
      ...
      U64 ptr = _convert_rank_to_ptr(shard, m);
      bool less = lexicographical_compare(
          input_buf, input_buf + num_bytes, shard.ds + ptr,
          shard.ds + min(ptr + num_bytes, shard.ds_size));
      if (less) {
        r = m;
      } else {
        l = m;
  ...
  void find_inplace(const vector<T> *const input_ids,
                    FindResult *const thread_output) const {
    *thread_output = find(*input_ids);
  }

  FindDisjResult find_disj(const vector<vector<T>> &disj_clause,
                           const U64 max_clause_freq) const {

    vector<FindResult> find_result_by_term(disj_clause.size());
    vector<thread> find_threads;
    for (size_t t = 0; t < disj_clause.size(); t++) {
      find_threads.emplace_back(&Engine::find_inplace, this, &disj_clause[t],
                                &find_result_by_term[t]);
    }
    ...
    U64 max_clause_freq_per_shard =
        (U64)((max_clause_freq + (_num_shards - 1)) / _num_shards); // ceil div
    for (size_t s = 0; s < _num_shards; s++) {
      threads.emplace_back(&Engine::_find_disj_thread, this, s,
                           &find_result_by_term, max_clause_freq_per_shard,
                           &cnt_by_shard[s], &segment_by_term_by_shard[s],
                           &subsampling_factor_by_shard[s]);
    }
    for (auto &thread : threads) {
      thread.join();
    }
  ...
  void _find_disj_thread(const size_t s,
                         const vector<FindResult> *const find_result_by_term,
                         const U64 max_clause_freq_per_shard, U64 *out_cnt,
    ...
    if (cnt > max_clause_freq_per_shard) {
      ...
      for (const auto &[start, end] : segment_by_term) {
        ...
        U64 new_length =
            (U64)((length * max_clause_freq_per_shard + (cnt - 1)) /
                  cnt); // ceil div
        std::uniform_int_distribution<U64> dis(
            0, length - new_length); // left inclusive, right inclusive
  ...
  void find_disj_inplace(const vector<vector<T>> *const disj_clause,
                         const U64 max_clause_freq,
                         FindDisjResult *const thread_output) const {
    *thread_output = find_disj(*disj_clause, max_clause_freq);
  }

  FindCnfResult find_cnf(const vector<vector<vector<T>>> &cnf,
                         const U64 max_clause_freq,
                         const U64 max_diff_tokens) const {
    ...
    for (size_t c = 0; c < cnf.size(); c++) {
      find_threads.emplace_back(&Engine::find_disj_inplace, this, &cnf[c],
                                max_clause_freq,
                                &find_disj_result_by_clause[c]);
    }
    for (auto &thread : find_threads) {
      thread.join();
    }
    ...
    for (size_t s = 0; s < _num_shards; s++) {
      threads.emplace_back(&Engine::_find_cnf_thread, this, s,
                           &find_disj_result_by_clause, max_diff_tokens,
                           &cnt_by_shard[s], &valid_ptr_ranges_by_shard[s],
                           &subsampling_factor_by_shard[s]);
    }
    for (auto &thread : threads) {
      thread.join();
    ...
    bool approx = any_of(subsampling_factor_by_shard.begin(),
                         subsampling_factor_by_shard.end(),
    ...
    return FindCnfResult{
        .cnt = cnt,
        .approx = approx,
        .ptrs_by_shard = ptrs_by_shard,
    };
  }
  ...
  void _find_cnf_thread(
      ...
      const U64 max_diff_tokens, U64 *out_cnt,
      vector<pair<U64, U64>> *out_valid_ptr_ranges,
      double *out_subsampling_factor) const {

    // sort find_disj_result_by_clause by cnt in of this shard in increasing
    // order
    vector<FindDisjResult> find_disj_result_by_clause =
        *_find_disj_result_by_clause;
    sort(find_disj_result_by_clause.begin(), find_disj_result_by_clause.end(),
         [s](const FindDisjResult &a, const FindDisjResult &b) {
    ...
    for (const auto &[start, end] :
         find_disj_result.segment_by_term_by_shard[s]) {
    ...
    // if there are Q terms and each term has M matches in the shard, the
    // complexity is O(Q * M * log(M))
    for (size_t d = 1; d < find_disj_result_by_clause.size(); d++) {
      ...
      for (const auto &[l, r] : valid_ptr_ranges) {
        ...
        auto hi =
            _bin_search(ptrs, l).second; // find the first match that is >= l
        U64 new_r = hi == ptrs.size() ? -1 : max(r, ptrs[hi]);
        if (new_l != (U64)-1 && new_l + max_diff_tokens * sizeof(T) >= l &&
            new_r != (U64)-1 &&
            new_r <=
                r + max_diff_tokens * sizeof(T)) { // +- MAX_DIFF_TOKENS tokens
    ...
    for (const auto &[l, r] : valid_ptr_ranges) {
      auto it =
          search(shard.ds + l, shard.ds + r, _doc_sep.begin(), _doc_sep.end());
      if (it == shard.ds + r) {
  ...
  void count_inplace(const vector<T> *const input_ids,
                     CountResult *const thread_output) const {
    *thread_output = count(*input_ids);
  }

  CountResult count_cnf(const vector<vector<vector<T>>> cnf,
                        const U64 max_clause_freq,
                        const U64 max_diff_tokens) const {
    auto find_cnf_result = find_cnf(cnf, max_clause_freq, max_diff_tokens);
    return CountResult{
        .count = find_cnf_result.cnt,
        .approx = find_cnf_result.approx,
    };
  }
  ...
  ProbResult prob(const vector<T> prompt_ids, const T cont_id) const {
    ...
    return ProbResult{
        .prompt_cnt = prompt_cnt, .cont_cnt = cont_cnt, .prob = prob};
  }
  ...
  DistResult<T> ntd(const vector<T> prompt_ids, const U64 max_support) const {
    ...
    vector<thread> threads;
    for (size_t s = 0; s < _num_shards; s++) {
      threads.emplace_back(&Engine::_get_freq_by_token_id_approx, this, s,
                           prompt_ids.size() * sizeof(T),
                           prompt_find_result.segment_by_shard[s], unit,
                           nullptr, nullptr, &freq_by_token_id_by_shard[s]);
    }
    for (auto &thread : threads) {
      thread.join();
    }
    ...
    map<T, DistTokenResult> result_by_token_id = {};
    for (const auto &[token_id, freq] : freq_by_token_id) {
      result_by_token_id[token_id] =
          DistTokenResult{freq, (double)freq / prompt_cnt};
    }
  ...
  void _get_freq_by_token_id_approx(
      const size_t s, const U64 num_bytes, const pair<U64, U64> segment,
      const U64 unit, const T *const token_start, const T *const token_end,
    ...
    if (end - start < 4 * unit) {
      for (U64 rank = start; rank < end; rank += unit) {
        U64 rank_mid =
            (rank + unit <= end) ? (rank + (unit >> 1)) : ((rank + end) >> 1);
        U64 ptr = _convert_rank_to_ptr(shard, rank_mid);
        T token_id = _convert_ptr_to_token_id(
            shard, _version == 4 ? ptr + num_bytes : ptr - sizeof(T));
        (*out_freq_by_token_id)[token_id] +=
            (rank + unit <= end) ? unit : (end - rank);
      }
      return;
    }

    // If start and end-1 has the same token, then we know this segment is all
    // the same token
    ...
    if (token_start) {
      ...
      new_token_start = _convert_ptr_to_token_id(
          shard, _version == 4 ? ptr_start + num_bytes : ptr_start - sizeof(T));
    }
    if (token_end) {
      new_token_end = *token_end;
    } else {
      U64 ptr_end = _convert_rank_to_ptr(shard, end - 1);
      new_token_end = _convert_ptr_to_token_id(
          shard, _version == 4 ? ptr_end + num_bytes : ptr_end - sizeof(T));
    }
    ...
    auto left_thread = thread(&Engine::_get_freq_by_token_id_approx, this, s,
                              num_bytes, left_segment, unit, &new_token_start,
                              nullptr, &left_thread_output);
    auto right_thread = thread(&Engine::_get_freq_by_token_id_approx, this, s,
                               num_bytes, right_segment, unit, nullptr,
                               &new_token_end, &right_thread_output);
    left_thread.join();
    right_thread.join();
    // TODO: This map merge is not efficient. Need to hack into the endianness
    // of token_ids.
  ...
  InfgramProbResult infgram_prob(const vector<T> prompt_ids,
                                 const T cont_id) const {
    ...
    while (true) {
      ...
      const vector<T> prompt_suffix_ids{prompt_ids.begin() + L - l_hi,
                                        prompt_ids.end()};
    ...
    while (l_hi - l_lo > 1) {
      ...
      const vector<T> prompt_suffix_ids{prompt_ids.begin() + L - l_mid,
                                        prompt_ids.end()};
      auto result = find(prompt_suffix_ids);
      if (result.cnt == 0) {
        l_hi = l_mid;
    ...
    const vector<T> prompt_suffix_ids{prompt_ids.begin() + L - suffix_len,
                                      prompt_ids.end()};
  ...
  InfgramDistResult<T> infgram_ntd(const vector<T> prompt_ids,
                                   const U64 max_support) const {
  ...
  SearchDocsResult<T> search_docs(const vector<T> input_ids,
                                  const size_t maxnum,
                                  const U64 max_disp_len) const {
    ...
    for (size_t d = 0; d < maxnum; d++) {
      size_t s = discrete_distribution<size_t>(cnt_by_shard.begin(),
                                               cnt_by_shard.end())(gen);
      auto &[start, end] = segment_by_shard[s];
      U64 rank = uniform_int_distribution<U64>(start, end - 1)(
          gen); // left inclusive, right inclusive
      U64 ptr = _convert_rank_to_ptr(_shards[s], rank);
      U64 idx =
          accumulate(cnt_by_shard.begin(), cnt_by_shard.begin() + s, (U64)0) +
          (rank - start);
  ...
  SearchDocsResult<T> search_docs_cnf(const vector<vector<vector<T>>> cnf,
                                      const size_t maxnum,
                                      const U64 max_disp_len,
                                      const U64 max_clause_freq,
                                      const U64 max_diff_tokens) const {
    ...
    U64 ptr_cnt =
        accumulate(ptr_cnt_by_shard.begin(), ptr_cnt_by_shard.end(), (U64)0);
    vector<U64> idxs;
    vector<DocResult<T>> docs;
    for (size_t d = 0; d < maxnum; d++) {
      size_t s = discrete_distribution<size_t>(ptr_cnt_by_shard.begin(),
                                               ptr_cnt_by_shard.end())(gen);
      auto &ptrs = ptrs_by_shard[s];
      U64 i = uniform_int_distribution<U64>(0, ptrs.size() - 1)(
          gen); // left inclusive, right inclusive
      auto &ptr = ptrs[i];
      double percentile =
          (double)(accumulate(ptr_cnt_by_shard.begin(),
                              ptr_cnt_by_shard.begin() + s, (U64)0) +
                   i) /
          ptr_cnt;
  ...
  DocResult<T> get_doc_by_rank(const size_t s, const U64 rank,
                               const U64 max_disp_len) const {
  ...
  void get_doc_by_rank_inplace(const size_t s, const U64 rank,
                               const U64 max_disp_len,
                               DocResult<T> *const thread_output) const {
    *thread_output = get_doc_by_rank(s, rank, max_disp_len);
  }

  vector<DocResult<T>>
  get_docs_by_ranks(const vector<pair<size_t, U64>> list_of_s_and_rank,
                    const U64 max_disp_len) const {
    ...
    for (size_t i = 0; i < list_of_s_and_rank.size(); i++) {
      threads.emplace_back(
          &Engine::get_doc_by_rank_inplace, this, list_of_s_and_rank[i].first,
          list_of_s_and_rank[i].second, max_disp_len, &docs[i]);
    }
    for (auto &thread : threads) {
      thread.join();
    }
  ...
  DocResult<T> get_doc_by_ptr(const size_t s, const U64 ptr,
                              const U64 max_disp_len) const {
    ...
    for (size_t _ = 0; _ < s; _++)
      doc_ix += _shards[_].doc_cnt;
    doc_ix += local_doc_ix;

    U64 doc_start_ptr =
        _convert_doc_ix_to_ptr(shard, local_doc_ix) +
        sizeof(T); // because we want to skip the document separator
    U64 doc_end_ptr = _convert_doc_ix_to_ptr(shard, local_doc_ix + 1);
    U64 doc_len = (doc_end_ptr - doc_start_ptr) / sizeof(T);

    U64 disp_start_ptr =
        max(doc_start_ptr, ptr < sizeof(T) * max_prepend_tokens
                               ? (U64)0
                               : (ptr - sizeof(T) * max_prepend_tokens));
    ...
    vector<T> token_ids(reinterpret_cast<T *>(shard.ds + disp_start_ptr),
                        reinterpret_cast<T *>(shard.ds + disp_end_ptr));
    if (_version == 5) {
  ...
  void get_doc_by_ptr_inplace(const size_t s, const U64 ptr,
                              const U64 max_disp_len,
                              DocResult<T> *const thread_output) const {
  ...
  vector<DocResult<T>>
  get_docs_by_ptrs(const vector<pair<size_t, U64>> list_of_s_and_ptr,
                   const U64 max_disp_len) const {

    vector<DocResult<T>> docs(list_of_s_and_ptr.size());
    vector<thread> threads;
    for (size_t i = 0; i < list_of_s_and_ptr.size(); i++) {
      threads.emplace_back(&Engine::get_doc_by_ptr_inplace, this,
                           list_of_s_and_ptr[i].first,
                           list_of_s_and_ptr[i].second, max_disp_len, &docs[i]);
    }
    for (auto &thread : threads) {
      thread.join();
    }
  ...
  DocResult<T> get_doc_by_ix(const U64 doc_ix, const U64 max_disp_len) const {
    ...
    U64 disp_end_ptr =
        min(doc_end_ptr, doc_start_ptr + sizeof(T) * max_disp_len);
    U64 disp_len = (disp_end_ptr - disp_start_ptr) / sizeof(T);
  ...
  void get_doc_by_ix_inplace(const U64 doc_ix, const U64 max_disp_len,
                             DocResult<T> *const thread_output) const {
    *thread_output = get_doc_by_ix(doc_ix, max_disp_len);
  }

  vector<DocResult<T>> get_docs_by_ixs(const vector<U64> list_of_doc_ix,
                                       const U64 max_disp_len) const {

    vector<DocResult<T>> docs(list_of_doc_ix.size());
    vector<thread> threads;
    for (size_t i = 0; i < list_of_doc_ix.size(); i++) {
      threads.emplace_back(&Engine::get_doc_by_ix_inplace, this,
                           list_of_doc_ix[i], max_disp_len, &docs[i]);
    }
    for (auto &thread : threads) {
      thread.join();
    }
  ...
  DocResult<T> get_doc_by_rank_2(const size_t s, const U64 rank,
                                 const U64 needle_len,
                                 const U64 max_ctx_len) const {
  ...
  void get_doc_by_rank_inplace_2(const size_t s, const U64 rank,
                                 const U64 needle_len, const U64 max_ctx_len,
                                 DocResult<T> *const thread_output) const {
    *thread_output = get_doc_by_rank_2(s, rank, needle_len, max_ctx_len);
  }

  vector<DocResult<T>> get_docs_by_ranks_2(
      const vector<tuple<size_t, U64, U64, U64>> requests) const {

    vector<DocResult<T>> docs(requests.size());
    vector<thread> threads;
    for (size_t i = 0; i < requests.size(); i++) {
      threads.emplace_back(&Engine::get_doc_by_rank_inplace_2, this,
                           get<0>(requests[i]), get<1>(requests[i]),
                           get<2>(requests[i]), get<3>(requests[i]), &docs[i]);
    }
    for (auto &thread : threads) {
      thread.join();
    }
  ...
  void get_docs_by_ranks_inplace_2(
      const vector<tuple<size_t, U64, U64, U64>> requests,
      vector<DocResult<T>> *const thread_output) const {
    *thread_output = get_docs_by_ranks_2(requests);
  }

  DocResult<T> get_doc_by_ptr_2(const size_t s, const U64 ptr,
                                const U64 needle_len,
                                const U64 max_ctx_len) const {
    ...
    U64 disp_start_ptr =
        max(doc_start_ptr, ptr < sizeof(T) * max_ctx_len
                               ? (U64)0
                               : (ptr - sizeof(T) * max_ctx_len));
    U64 disp_end_ptr =
        min(doc_end_ptr, ptr + sizeof(T) * (needle_len + max_ctx_len));
    U64 disp_len = (disp_end_ptr - disp_start_ptr) / sizeof(T);
    U64 needle_offset = (ptr - disp_start_ptr) / sizeof(T);
  ...
  void get_doc_by_ptr_inplace_2(const size_t s, const U64 ptr,
                                const U64 needle_len, const U64 max_ctx_len,
                                DocResult<T> *const thread_output) const {
    *thread_output = get_doc_by_ptr_2(s, ptr, needle_len, max_ctx_len);
  }

  vector<DocResult<T>> get_docs_by_ptrs_2(
      const vector<tuple<size_t, U64, U64, U64>> requests) const {

    vector<DocResult<T>> docs(requests.size());
    vector<thread> threads;
    for (size_t i = 0; i < requests.size(); i++) {
      threads.emplace_back(&Engine::get_doc_by_ptr_inplace_2, this,
                           get<0>(requests[i]), get<1>(requests[i]),
                           get<2>(requests[i]), get<3>(requests[i]), &docs[i]);
    }
    for (auto &thread : threads) {
  ...
  void get_docs_by_ptrs_inplace_2(
      const vector<tuple<size_t, U64, U64, U64>> requests,
      vector<DocResult<T>> *const thread_output) const {
  ...
  void get_doc_by_ix_inplace_2(const U64 doc_ix, const U64 max_disp_len,
                               DocResult<T> *const thread_output) const {
    *thread_output = get_doc_by_ix_2(doc_ix, max_disp_len);
  }

  vector<DocResult<T>>
  get_docs_by_ixs_2(const vector<tuple<U64, U64>> requests) const {

    vector<DocResult<T>> docs(requests.size());
    vector<thread> threads;
    for (size_t i = 0; i < requests.size(); i++) {
      threads.emplace_back(&Engine::get_doc_by_ix_inplace_2, this,
                           get<0>(requests[i]), get<1>(requests[i]), &docs[i]);
    }
    for (auto &thread : threads) {
      thread.join();
    }
  ...
  size_t get_num_shards() const { return _num_shards; }
  ...
  size_t get_lcp_len(const U8 *const a, const size_t len_a, const U8 *const b,
                     const size_t len_b) const {
    size_t i = 0;
  ...
  void compute_longest_prefix_len_thread(
      const vector<T> *const input_ids, const size_t s,
      size_t *const out_longest_prefix_len) const {
    ...
    *out_longest_prefix_len = 0;
    // Inspect start-1 and start, but skip start-1 if start == 0, and skip start
    // if start == shard.tok_cnt
    for (U64 rank = max((U64)0, start - 1);
         rank < min(shard.tok_cnt, start + 1); rank++) {
      U64 ptr = _convert_rank_to_ptr(shard, rank);
      size_t prefix_len =
          get_lcp_len(shard.ds + ptr, shard.ds_size - ptr, input_buf,
                      input_ids->size() * sizeof(T)) /
          sizeof(T);
  ...
  virtual size_t
  compute_longest_prefix_len(const vector<T> &input_ids,
                             const vector<T> &delim_ids,
                             const bool enforce_bow = false) const {
    ...
    for (size_t s = 0; s < _num_shards; s++) {
      longest_prefix_len =
          max(longest_prefix_len, longest_prefix_len_by_shard[s]);
    }

    // cut off at the first occurrence of a delimiter
    if (!delim_ids.empty()) {
      for (size_t pos = 0; pos + 1 < longest_prefix_len; pos++) {
        if (std::find(delim_ids.begin(), delim_ids.end(), input_ids[pos]) !=
            delim_ids.end()) {
    ...
    if (enforce_bow) {
      while (longest_prefix_len > 0) {
        if (longest_prefix_len == input_ids.size() ||
            _bow_ids.find(input_ids[longest_prefix_len]) != _bow_ids.end()) {
          break;
        }
        longest_prefix_len--;
      }
  ...
  void creativity_thread(const vector<T> *const input_ids, const size_t l,
                         size_t *const out_r) const {
  ...
  CreativityResult creativity(const vector<T> input_ids) const {
    ...
    for (size_t l = 0; l < input_ids.size(); l++) {
      threads.emplace_back(&Engine::creativity_thread, this, &input_ids, l,
                           &rs[l]);
    }
    for (auto &thread : threads) {
      thread.join();
  ...
  void compute_interesting_spans_thread(
      const vector<T> *const input_ids, const size_t l,
      const vector<T> *const delim_ids, const size_t min_len,
      const size_t max_cnt, const bool enforce_bow,
      vector<pair<PSS, FindResult>> *const out_span_find_pairs) const {

    vector<T> suffix_ids(input_ids->begin() + l, input_ids->end());
    size_t len =
        compute_longest_prefix_len(suffix_ids, *delim_ids, enforce_bow);
    if (len < min_len)
      return;
    vector<T> span_ids(input_ids->begin() + l, input_ids->begin() + l + len);
    auto find_result = find(span_ids);
    if (find_result.cnt > max_cnt)
      return;
    out_span_find_pairs->push_back({{l, l + len}, find_result});
  }

  vector<pair<PSS, FindResult>>
  compute_interesting_spans(const vector<T> &input_ids,
                            const vector<T> &delim_ids, const size_t min_len,
                            const size_t max_cnt,
                            const bool enforce_bow) const {

    vector<vector<pair<PSS, FindResult>>> span_find_pairs_by_l(
        input_ids.size());
    for (size_t l_block = 0; l_block < input_ids.size();
         l_block += _attribution_block_size) {
      vector<thread> threads;
      for (size_t l = l_block;
           l < min(l_block + _attribution_block_size, input_ids.size()); l++) {
        // skip if this word is not a beginning-of-word
        if (enforce_bow && _bow_ids.find(input_ids[l]) == _bow_ids.end())
          continue;

        threads.emplace_back(&Engine::compute_interesting_spans_thread, this,
                             &input_ids, l, &delim_ids, min_len, max_cnt,
                             enforce_bow, &span_find_pairs_by_l[l]);
      }
      for (auto &thread : threads) {
        thread.join();
      }
    ...
    for (const auto &span_find_pairs : span_find_pairs_by_l) {
      flattened_span_find_pairs.insert(flattened_span_find_pairs.end(),
                                       span_find_pairs.begin(),
                                       span_find_pairs.end());
    }
  ...
  void get_attribution_span_thread(
      const pair<PSS, FindResult> *const span_find_pair,
      AttributionSpan *const out_attribution_span) const {
  ...
  AttributionResult attribute(const vector<T> input_ids,
                              const vector<T> delim_ids, const size_t min_len,
                              const size_t max_cnt,
                              const bool enforce_bow) const {

    vector<pair<PSS, FindResult>> span_find_pairs = compute_interesting_spans(
        input_ids, delim_ids, min_len, max_cnt, enforce_bow);

    vector<AttributionSpan> attribution_spans(span_find_pairs.size());
    vector<thread> threads;
    for (size_t i = 0; i < span_find_pairs.size(); i++) {
      threads.emplace_back(&Engine::get_attribution_span_thread, this,
                           &span_find_pairs[i], &attribution_spans[i]);
    }
    for (auto &thread : threads) {
      thread.join();
    }
  ...
  FindResult find_1approx2(const std::vector<T> input_ids) const {
    ...
    for (size_t s = 0; s < _num_shards; s++) {
      threads.emplace_back(&Engine::_find_1approx2_thread, this, s, a, b,
                           &segment_by_shard[s]);
    }
    for (auto &t : threads)
      t.join();
  ...
  FindResult find_2approx3(const std::vector<T> input_ids) const {
    ...
    std::vector<std::thread> threads;
    for (size_t s = 0; s < _num_shards; s++) {
      threads.emplace_back(&Engine::_find_2approx3_thread, this, s, a, b, c,
                           &segment_by_shard[s]);
    }
    for (auto &t : threads)
      t.join();
...
private:
  ...
  // Finds the smallest rank r in [lo_bnd, hi_bnd] such that SA[r] is NOT "left
  // of boundary":
  //   strict=false -> boundary is "first SA[r] >= query"  (left edge of match
  //   range) strict=true  -> boundary is "first SA[r] >  query"  (right edge of
  //   match range)
  // Returns hi_bnd if every SA entry in the range is strictly left of the
  // boundary.
  U64 _exp_search_boundary(const DatastoreShard &shard,
                           const U8 *const input_buf, const U64 num_bytes,
                           U64 est, const U64 lo_bnd, const U64 hi_bnd,
    ...
    auto is_left = [&](U64 r) -> bool {
      ...
      if (strict) {
        ...
        return !std::lexicographical_compare(input_buf, input_buf + num_bytes,
                                             sa_begin, sa_end);
      } else {
        // SA[r] < query
        return std::lexicographical_compare(sa_begin, sa_end, input_buf,
                                            input_buf + num_bytes);
      }
    };
    ...
    U64 lo, hi; // post-bracket invariant: is_left(lo), !is_left(hi) OR hi ==
                // hi_bnd sentinel
    ...
    if (is_left(est)) {
      ...
      if (!found_lo)
        return lo_bnd; // nothing in [lo_bnd, est] is left of boundary
    }
  ...
  void _find_1approx2_thread(const size_t s, const T a, const T b,
    ...
    if (uni_a_lo >= uni_a_hi) {
      *out_segment = {uni_a_lo, uni_a_lo};
      return;
    }

    // Approximation: within uni(a) range, 2nd-token positions follow global
    // unigram CDF
    U64 total = shard.tok_cnt;
    U64 range_a = uni_a_hi - uni_a_lo;
    U64 est_lo =
        uni_a_lo + (U64)((long double)range_a * cum[b] / (long double)total);
    U64 est_hi = uni_a_lo +
                 (U64)((long double)range_a * cum[b + 1] / (long double)total);
    if (est_lo < uni_a_lo)
      est_lo = uni_a_lo;
    if (est_hi > uni_a_hi)
      est_hi = uni_a_hi;
    ...
    U64 left = _exp_search_boundary(shard, input_buf, num_bytes, est_lo,
                                    uni_a_lo, uni_a_hi, false);
    U64 right = _exp_search_boundary(shard, input_buf, num_bytes, est_hi,
                                     uni_a_lo, uni_a_hi, true);
    if (right < left)
      right = left;
  ...
  void _find_2approx3_thread(const size_t s, const T a, const T b, const T c,
                             std::pair<U64, U64> *const out_segment) const {
    ...
    U64 est_lo =
        bi_lo + (U64)((long double)range_ab * cum[c] / (long double)total);
    U64 est_hi =
        bi_lo + (U64)((long double)range_ab * cum[c + 1] / (long double)total);
    if (est_lo < bi_lo)
      est_lo = bi_lo;
    if (est_hi > bi_hi)
      est_hi = bi_hi;
    ...
    U64 left = _exp_search_boundary(shard, input_buf, num_bytes, est_lo, bi_lo,
                                    bi_hi, false);
    U64 right = _exp_search_boundary(shard, input_buf, num_bytes, est_hi, bi_lo,
                                     bi_hi, true);
    if (right < left)
      right = left;
  ...
  void _prefetch_find(const DatastoreShard &shard, const U64 num_bytes,
                      const U64 lo, const U64 hi,
                      const size_t depth = 0) const {
    U64 mi = (lo + hi) >> 1;
    if (mi >= shard.tok_cnt)
      return; // This might happen when lo = -1 and hi = 0
    if (_ds_prefetch_depth != 0 && depth == _ds_prefetch_depth) { // fetch ds
      U64 ptr = _convert_rank_to_ptr(shard, mi);
      madvise(shard.ds + ptr - ptr % PAGESIZE, num_bytes + ptr % PAGESIZE,
              MADV_WILLNEED);
    }
    if (_ds_prefetch_depth != _sa_prefetch_depth &&
        depth == _sa_prefetch_depth) { // fetch sa
      madvise(shard.sa + mi * shard.ptr_size - mi * shard.ptr_size % PAGESIZE,
              shard.ptr_size + mi * shard.ptr_size % PAGESIZE, MADV_WILLNEED);
    }
    if (depth == _sa_prefetch_depth)
      return;
    _prefetch_find(shard, num_bytes, lo, mi, depth + 1);
    _prefetch_find(shard, num_bytes, mi, hi, depth + 1);
  }

  void _prefetch_find_2(const DatastoreShard &shard, const U64 num_bytes,
                        const U64 lo, const U64 hi,
                        const size_t depth = 0) const {
    U64 mi = (lo + hi - 1) >> 1;
    if (mi >= shard.tok_cnt)
      return; // This might happen when lo = 0 and hi = 0
    if (_ds_prefetch_depth != 0 && depth == _ds_prefetch_depth) { // fetch ds
      U64 ptr = _convert_rank_to_ptr(shard, mi);
      madvise(shard.ds + ptr - ptr % PAGESIZE, num_bytes + ptr % PAGESIZE,
              MADV_WILLNEED);
  ...
  void _prefetch_ntd(const DatastoreShard &shard, const U64 num_bytes,
                     const U64 lo, const U64 hi, const size_t depth = 0) const {
    U64 mi = (lo + hi) >> 1;
    if (mi >= shard.tok_cnt)
      return; // This might happen when lo = -1 and hi = 0
    if (_ds_prefetch_depth != 0 && depth == _ds_prefetch_depth) { // fetch ds
      U64 ptr = _version == 4
                    ? (_convert_rank_to_ptr(shard, mi - 1) + num_bytes)
                    : (_convert_rank_to_ptr(shard, mi - 1) - sizeof(T));
      madvise(shard.ds + ptr - ptr % PAGESIZE, sizeof(T) + ptr % PAGESIZE,
              MADV_WILLNEED);
      ptr = _version == 4 ? (_convert_rank_to_ptr(shard, mi) + num_bytes)
                          : (_convert_rank_to_ptr(shard, mi) - sizeof(T));
      madvise(shard.ds + ptr - ptr % PAGESIZE, sizeof(T) + ptr % PAGESIZE,
              MADV_WILLNEED);
    }
    if (_ds_prefetch_depth != _sa_prefetch_depth &&
        depth == _sa_prefetch_depth) { // fetch sa
      madvise(shard.sa + (mi - 1) * shard.ptr_size -
                  (mi - 1) * shard.ptr_size % PAGESIZE,
              2 * shard.ptr_size + (mi - 1) * shard.ptr_size % PAGESIZE,
              MADV_WILLNEED); // since we need both mi-1 and mi
    }
    if (depth == _sa_prefetch_depth)
      return;
  ...
  void _prefetch_doc(const DatastoreShard &shard, const U64 lo, const U64 hi,
                     const size_t depth = 0) const {
    if (_od_prefetch_depth == 0)
      return;
    U64 mi = (lo + hi) >> 1;
    if (mi >= shard.doc_cnt)
      return; // This might happen when lo = -1 and hi = 0
    if (depth == _od_prefetch_depth) { // fetch od
      madvise(shard.od + mi * sizeof(U64) - mi * sizeof(U64) % PAGESIZE,
              sizeof(U64) + mi * sizeof(U64) % PAGESIZE, MADV_WILLNEED);
      return;
    }
  ...
  inline T _convert_ptr_to_token_id(const DatastoreShard &shard,
                                    const U64 ptr) const {
    assert(ptr % sizeof(T) == 0);
    assert(ptr <= shard.ds_size);
    ...
    // If you see \xff\xff, this actually means we're at the very end of a
    // document.
    if (token_id == _doc_sep_id)
      token_id = _eos_token_id;
    return token_id;
  }

  inline U64 _convert_rank_to_ptr(const DatastoreShard &shard,
                                  const U64 rank) const {
    assert(rank < shard.tok_cnt);
  ...
  inline vector<U64> _convert_ranks_to_ptrs(const DatastoreShard &shard,
                                            const U64 rank_start,
                                            const U64 rank_end) const {
  ...
  inline U64 _convert_doc_ix_to_ptr(const DatastoreShard &shard,
                                    const U64 doc_ix) const {
  ...
  inline U64 _convert_doc_ix_to_meta_ptr(const DatastoreShard &shard,
                                         const U64 doc_ix) const {
    assert(doc_ix <= shard.doc_cnt);
    if (doc_ix == shard.doc_cnt) {
...
template <typename T> class EngineDiff : public Engine<T> {

public:
  EngineDiff(const vector<string> index_dirs,
             const vector<string> index_dirs_diff, const T eos_token_id,
             const T vocab_size, const size_t version, const bool load_to_ram,
             const size_t ds_prefetch_depth, const size_t sa_prefetch_depth,
             const size_t od_prefetch_depth, const set<T> bow_ids,
             const size_t attribution_block_size,
             const bool precompute_unigram_logprobs,
             map<string, vector<DatastoreShard>> prev_shards_by_index_dir)
      : Engine<T>(index_dirs, eos_token_id, vocab_size, version, load_to_ram,
                  ds_prefetch_depth, sa_prefetch_depth, od_prefetch_depth,
                  bow_ids, attribution_block_size, precompute_unigram_logprobs,
                  prev_shards_by_index_dir),
        _engine_diff(make_unique<Engine<T>>(
            index_dirs_diff, eos_token_id, vocab_size, version, load_to_ram,
            ds_prefetch_depth, sa_prefetch_depth, od_prefetch_depth, bow_ids,
            attribution_block_size, precompute_unigram_logprobs,
            prev_shards_by_index_dir)) {}

  // The shape of returned document results is identical to the shape of input
  // requests. Blocked documents are marked and have an empty token_ids.
  vector<vector<DocResult<T>>> get_docs_by_ptrs_2_grouped(
      const vector<tuple<vector<pair<size_t, U64>>, vector<T>, U64, U64>>
          requests) const {
    ...
    for (size_t r = 0; r < requests.size(); r++) {
      ...
      threads.emplace_back(&Engine<T>::get_docs_by_ptrs_inplace_2, this,
                           requests_main, &docs_main_by_request[r]);
      ...
      threads.emplace_back(&Engine<T>::get_docs_by_ranks_inplace_2,
                           _engine_diff.get(), requests_diff,
                           &docs_diff_by_request[r]);
    }
    for (auto &thread : threads) {
      thread.join();
    }
    ...
    for (size_t r = 0; r < requests.size(); r++) {
      ...
      for (auto &doc_main : docs_main) {
        // if doc_main.token_ids match any in the diff index, block this
        // document
        if (any_of(docs_diff.begin(), docs_diff.end(),
                   [&](const DocResult<T> &doc_diff) {
  ...
  compute_longest_prefix_len(const vector<T> &input_ids,
                             const vector<T> &delim_ids,
                             const bool enforce_bow = false) const override {

    size_t longest_prefix_len = Engine<T>::compute_longest_prefix_len(
        input_ids, delim_ids, enforce_bow);

    // consider docs in the diff index
    while (longest_prefix_len > 0) {
      auto count_main =
          Engine<T>::count(
              {input_ids.begin(), input_ids.begin() + longest_prefix_len})
              .count;
      auto count_diff = _engine_diff
                            ->count({input_ids.begin(),
                                     input_ids.begin() + longest_prefix_len})
                            .count;
      if (count_main > count_diff)
        break;
      longest_prefix_len--;
    }
